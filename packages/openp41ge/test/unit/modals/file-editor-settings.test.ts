...
describe("openp41ge-file-editor-settings", () => {
  ...
  test("word-wrap card renders a switch, defaulting to OFF when unset", async () => {
    const fake = new FakeConfig({});
    const el = await mount(fake);
    const sw = el.querySelector("openp41ge-toggle");
    expect(sw).not.toBeNull();
    expect(el.textContent).toContain("Word wrap for new files");
    expect(sw.checked).toBe(false);
    expect(sw.shadowRoot.querySelector(".label").textContent).toBe("Off");
  });
  ...
  test("word-wrap switch reflects the saved default", async () => {
    ...
    expect(sw.shadowRoot.querySelector(".label").textContent).toBe("On");
  });
  ...
  test("toggling the word-wrap switch persists a boolean to editor.wordWrap", async () => {
    ...
    const el = await mount(fake);
    const sw = el.querySelector("openp41ge-toggle");
    sw.shadowRoot.querySelector("input").checked = true;
    sw.shadowRoot.querySelector("input").dispatchEvent(new Event("change", { bubbles: true }));
  ...
  test("word-wrap switch reacts to an external config change", async () => {
    const fake = new FakeConfig({});
    const el = await mount(fake);
    expect(el.querySelector("openp41ge-toggle").checked).toBe(false);

    await fake.set("editor.wordWrap", true);
    await new Promise((r) => setTimeout(r, 10));
    expect(el.querySelector("openp41ge-toggle").checked).toBe(true);
  });
});
