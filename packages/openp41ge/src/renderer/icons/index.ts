...
/** Search (Material magnifier + list lines) icon — for the sidebar bottom-bar search tools.
 * Uses currentColor so the button's colour controls the icon (grey off,
 * white on hover / when the search drawer is open). */
export function searchIcon(size?: number): string {
  const s = size ?? 24;
